      ...
      {course.units.map((unit, unitIdx) => (
        <div key={unit.id} className="w-full max-w-2xl flex flex-col items-center">
          ...
          <div className="w-full mb-6 rounded-3xl border border-slate-200/70 bg-white/80 p-5 text-center shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/40 transition-colors relative overflow-hidden">
            ...
            <div className="text-xs font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-400">Unit {unitIdx + 1}</div>
            <h2 className="mt-1 font-serif text-2xl font-bold text-slate-900 dark:text-white">{unit.title}</h2>
                ...
                >
                  <button
                    ...
                    className={`
                      ...
                      ${isCompleted ? 'bg-hermetic-success text-white' :
                        isCurrent ? 'bg-hermetic-accent text-slate-950' :
                  ...
                  >
                    ...
                    {isCurrent && <Star className="w-10 h-10 text-slate-950 fill-slate-950 animate-pulse" />}
